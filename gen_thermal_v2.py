"""
Generate thermal_agent_v2.ipynb — Publication-quality improvements:

ROOT CAUSE FIXES:
  1. Dynamic Q_REF: per-episode reference (Q_pred at step 0) instead of global mean
     → degradation penalty NOW fires when temperature hurts capacity
  2. TD3 (Twin-Delayed DDPG) replaces vanilla DDPG → more stable, less overestimation
  3. Noise decay: OU sigma decays from 0.3 → 0.05 over training
  4. State dim 7: adds ΔQ feature (instantaneous capacity gradient)
  5. 1500 training episodes (vs 500)
  6. β ablation: 300 episodes per β (vs 100) → differentiates agents
  7. STEPS_PER_EP = 300 (vs 200) → longer cycles, better surrogate coverage
  8. No-surrogate DDPG (β=0) as explicit ablation baseline
  9. LR scheduling for both actor and critic
 10. 50-episode final evaluation (vs 20) → tighter error bars
"""
import json, os

BASE = "/Users/mahizhan/Documents/Sem7/Github/data-driven-prediction-of-battery-cycle-life-before-capacity-degradation"
_id = [0]
def uid(): _id[0]+=1; return f"v2-{_id[0]:04d}"
...
# ── Cell 2: Surrogate ─────────────────────────────────────────────────────────
cells.append(md("## 2 · Load Surrogate"))
cells.append(code([
...
# ── Cell 3: Load data ─────────────────────────────────────────────────────────
cells.append(md("## 3 · Load Data & Build Profiles"))
cells.append(code([
...
# ── Cell 8: Trajectories ──────────────────────────────────────────────────────
cells.append(md("## 8 · Fixed-Profile Trajectory Collection"))
cells.append(code([
    "random.seed(SEED)\n",
...
out=os.path.join(BASE,"thermal_agent_v2.ipynb")
